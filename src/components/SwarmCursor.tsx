...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    const compProgram = new Program(gl, {
      ...
      uniforms: {
        tField: { value: null },
        uColor: { value: hexToRgb(propsRef.current.color as string) },
        uAccent: { value: hexToRgb(propsRef.current.accentColor as string) },
    ...
    const frame = (now: number) => {
      ...
      const p = propsRef.current as Required<Omit<SwarmCursorProps, "children" | "className" | "style">>;
      ...
      compProgram.uniforms.tField.value = (target as RenderTarget).texture;
      renderer.render({ scene: compMesh });
    };
    raf = requestAnimationFrame(frame);
